"""CLI for zo-repo-task."""

import argparse
import json
import os
import sys
import textwrap
from datetime import datetime, timezone
from typing import Any

import requests


DEFAULT_BASE_URL = "https://api.github.com"


def text_value(value: Any, fallback: str = "") -> str:
    """Return a string field from an API payload without trusting its type."""
    return value if isinstance(value, str) else fallback


def response_items(response: requests.Response, resource: str) -> list[dict[str, Any]]:
    """Validate a GitHub response that should contain a list of objects."""
    payload = response.json()
    if not isinstance(payload, list) or not all(isinstance(item, dict) for item in payload):
        raise ValueError(f"GitHub returned an invalid {resource} response")
    return payload


def response_search_items(response: requests.Response) -> list[dict[str, Any]]:
    """Extract and validate repository search results."""
    payload = response.json()
    if not isinstance(payload, dict) or not isinstance(payload.get("items"), list):
        raise ValueError("GitHub returned an invalid repository search response")
    if not all(isinstance(item, dict) for item in payload["items"]):
        raise ValueError("GitHub returned invalid repository search items")
    return payload["items"]


def validate_limit(limit: int) -> int:
    """Return a usable API page size or reject an invalid one."""
    if isinstance(limit, bool) or not isinstance(limit, int):
        raise TypeError("limit must be an integer")
    if limit < 1:
        raise ValueError("limit must be at least 1")
    return min(limit, 100)


def make_session() -> requests.Session:
    """Create a requests session with appropriate headers."""
    session = requests.Session()
    session.headers["Accept"] = "application/vnd.github+json"
    token = os.environ.get("GITHUB_TOKEN")
    if token:
        session.headers["Authorization"] = f"Bearer {token}"
    return session


def list_repos(user: str, session: requests.Session, limit: int = 30) -> list[dict[str, Any]]:
    """List repositories for a GitHub user, sorted by recently pushed."""
    url = f"{DEFAULT_BASE_URL}/users/{user}/repos"
    params = {"sort": "pushed", "per_page": validate_limit(limit), "type": "owner"}
    response = session.get(url, params=params, timeout=15)
    response.raise_for_status()
    return response_items(response, "repository list")


def get_repo(owner: str, repo: str, session: requests.Session):
    """Get repository details."""
    url = f"{DEFAULT_BASE_URL}/repos/{owner}/{repo}"
    response = session.get(url, timeout=15)
    response.raise_for_status()
    payload = response.json()
    if not isinstance(payload, dict):
        raise ValueError("GitHub returned an invalid repository response")
    return payload


def search_repos(query: str, session: requests.Session, limit: int = 30):
    """Search repositories by keyword."""
    url = f"{DEFAULT_BASE_URL}/search/repositories"
    params = {"q": query, "sort": "stars", "per_page": validate_limit(limit)}
    response = session.get(url, params=params, timeout=15)
    response.raise_for_status()
    return response_search_items(response)


def format_repo_text(repo: dict[str, Any], indent: int = 0) -> str:
    """Format a single repository as human-readable text."""
    prefix = "  " * indent
    name = text_value(repo.get("full_name"), "?") or "?"
    description = text_value(repo.get("description"))
    language = text_value(repo.get("language"), "?") or "?"
    pushed = text_value(repo.get("pushed_at"), "?") or "?"
    lines = [f"{prefix}{name}"]
    if description:
        lines.append(f"{prefix}  {textwrap.shorten(description, width=70)}")
    lines.append(
        f"{prefix}  stars={repo.get('stargazers_count', 0)} "
        f"forks={repo.get('forks_count', 0)} "
        f"lang={language} "
        f"pushed={pushed[:10]}"
    )
    return "\n".join(lines)


def format_repos_table(repos: list[dict[str, Any]]) -> str:
    """Format a list of repositories as an ASCII table."""
    if not repos:
        return "(no repositories)"

    names = [text_value(repo.get("full_name"), "?") or "?" for repo in repos]
    descriptions = [text_value(repo.get("description")) for repo in repos]
    name_width = max(map(len, names))
    desc_width = min(50, max(1, max(map(len, descriptions))))

    header = f"{'Name':<{name_width}} {'Stars':>6} {'Forks':>6} {'Language':<12} {'Pushed'}"
    sep = "-" * len(header)
    rows = [header, sep]

    for repo in repos:
        name = text_value(repo.get("full_name"), "?") or "?"
        stars = str(repo.get("stargazers_count", 0))
        forks = str(repo.get("forks_count", 0))
        lang = (text_value(repo.get("language"), "?") or "?")[:12]
        pushed = (text_value(repo.get("pushed_at"), "?") or "?")[:10]
        raw_description = text_value(repo.get("description"))
        desc = textwrap.shorten(raw_description, width=desc_width) if raw_description else ""
        rows.append(f"{name:<{name_width}} {stars:>6} {forks:>6} {lang:<12} {pushed}")

        if desc:
            rows.append(f"{'':>{name_width + 70}}  {desc}")

    return "\n".join(rows)


def cmd_list(args: argparse.Namespace) -> None:
    """Handle the list command."""
    session = make_session()
    try:
        repos = list_repos(args.user, session, limit=args.limit)
    except requests.HTTPError as e:
        print(f"Error: {e}", file=sys.stderr)
        sys.exit(1)

    if not repos:
        print(f"No repositories found for user: {args.user}")
        sys.exit(0)

    if args.format == "json":
        print(json.dumps(repos, indent=2))
    else:
        print(format_repos_table(repos))


def cmd_info(args: argparse.Namespace) -> int:
    """Handle the info command."""
    repo_ref = args.repo.strip().strip("/")
    if repo_ref.count("/") != 1 or any(not part for part in repo_ref.split("/")):
        print("Error: repository must use owner/name format", file=sys.stderr)
        return 1

    session = make_session()
    owner, repo = repo_ref.split("/", 1)
    try:
        data = get_repo(owner, repo, session)
    except requests.HTTPError as e:
        print(f"Error: {e}", file=sys.stderr)
        sys.exit(1)

    if args.format == "json":
        print(json.dumps(data, indent=2))
    else:
        print(format_repo_text(data))
        print()
        topics = data.get("topics", [])
        if isinstance(topics, list):
            topic_names = [topic for topic in topics if isinstance(topic, str) and topic]
            if topic_names:
                print(f"  Topics: {', '.join(topic_names)}")
    return 0


def cmd_search(args: argparse.Namespace) -> None:
    """Handle the search command."""
    session = make_session()
    try:
        results = search_repos(args.query, session, limit=args.limit)
    except requests.HTTPError as e:
        print(f"Error: {e}", file=sys.stderr)
        sys.exit(1)

    if not results:
        print(f"No repositories found matching: {args.query}")
        sys.exit(0)

    if args.format == "json":
        print(json.dumps(results, indent=2))
    else:
        print(format_repos_table(results))


def main(argv: list[str] | None = None) -> int:
    """Main CLI entry point."""
    parser = argparse.ArgumentParser(
        prog="zo-task",
        description="GitHub repository explorer CLI",
    )
    sub = parser.add_subparsers(dest="command", required=True)

    list_p = sub.add_parser("list", help="List repositories for a user")
    list_p.add_argument("--user", "-u", default=os.environ.get("GITHUB_USER", ""),
                       help="GitHub username (default: GITHUB_USER env var)")
    list_p.add_argument("--limit", "-l", type=int, default=30)
    list_p.add_argument("--format", "-f", choices=["text", "json"], default="text")
    list_p.set_defaults(func=cmd_list)

    info_p = sub.add_parser("info", help="Show repository details")
    info_p.add_argument("repo", help="Repository in owner/name format")
    info_p.add_argument("--format", "-f", choices=["text", "json"], default="text")
    info_p.set_defaults(func=cmd_info)

    search_p = sub.add_parser("search", help="Search repositories")
    search_p.add_argument("query", help="Search query")
    search_p.add_argument("--limit", "-l", type=int, default=30)
    search_p.add_argument("--format", "-f", choices=["text", "json"], default="text")
    search_p.set_defaults(func=cmd_search)

    args = parser.parse_args(argv)

    if not hasattr(args, "func"):
        parser.print_help()
        return 0

    if args.command in ("list", "search") and not getattr(args, "user", None) and not getattr(args, "query", None):
        print(f"Error: --user required for '{args.command}' command", file=sys.stderr)
        return 1

    try:
        result = args.func(args)
    except (requests.RequestException, ValueError) as e:
        print(f"Request error: {e}", file=sys.stderr)
        return 1
    return result if isinstance(result, int) else 0


if __name__ == "__main__":
    sys.exit(main())
